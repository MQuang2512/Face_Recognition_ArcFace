...
# Add a lock for thread synchronization
...
def process_tracking(frame, detector, tracker, args, frame_id, fps):
    ...
    # Use lock when updating shared data
    with data_lock:
        data_mapping["raw_image"] = img_info["raw_img"]
        data_mapping["detection_bboxes"] = bboxes if bboxes is not None else np.array([])
        data_mapping["detection_landmarks"] = landmarks if landmarks is not None else np.array([])
...
def tracking(detector, args):
    ...
    cap = cv2.VideoCapture(1) #Default camera=0 (1,2,3,...) 
...
def recognize():
    ...
    while True:
        ...
        if (len(raw_image) == 0 or len(tracking_bboxes) == 0 or 
            len(detection_bboxes) == 0 or len(detection_landmarks) == 0):
            time.sleep(0.01)  # Avoid high CPU usage in the loop
            continue
            ...
            for i in range(len(tracking_bboxes)):
                for j in range(len(detection_bboxes)):
                    ...
                    if mapping_score > 0.5:
                        try:
                            # Check if landmarks exist and are valid before using them
                            if j < len(detection_landmarks) and len(detection_landmarks[j]) > 0:
                            ...
                            # Remove the processed detection to avoid duplicate matching
                            with data_lock:
                                if len(detection_bboxes) > j:
                                    detection_bboxes = np.delete(detection_bboxes, j, axis=0)
                                if len(detection_landmarks) > j:
                                    detection_landmarks = np.delete(detection_landmarks, j, axis=0)
                                    
                                # Update the data_mapping with modified arrays
                                data_mapping["detection_bboxes"] = detection_bboxes
                                data_mapping["detection_landmarks"] = detection_landmarks
                            
                            break
                        except IndexError as e:
                            print(f"IndexError in recognition: {e}")
                            continue
                        except Exception as e:
                            print(f"Error in recognition: {e}")
                            continue
        except Exception as e:
            print(f"Error in recognize thread: {e}")
            time.sleep(0.01)
...
def main():
    ...
    thread_track.daemon = True  # Make thread exit when main program exits
    thread_track.start()

    # Start recognition thread
    thread_recognize = threading.Thread(target=recognize)
    thread_recognize.daemon = True  # Make thread exit when main program exits
    thread_recognize.start()
    
    try:
        # Keep main thread alive
        while thread_track.is_alive() and thread_recognize.is_alive():
            time.sleep(0.1)
